...
def _variable(name, shape, initializer):
  ...
  """
    ...
    var = tf.compat.v1.compat.v1.get_variable(name, shape, initializer=initializer)
    return var
...
def conv_layer(inputs, kernel_size, stride, num_features, idx, nonlinearity=None):
    with tf.compat.v1.variable_scope('{0}_conv'.format(idx)) as scope:
        ...
        biases = _variable('biases', [num_features], initializer=tf.keras.initializers.glorot_normal())
...
def fc_layer(inputs, hiddens, idx, nonlinearity=None, flat=False):
    with tf.compat.v1.variable_scope('{0}_fc'.format(idx)) as scope:
        ...
        weights = _variable('weights', shape=[dim, hiddens], initializer=tf.keras.initializers.glorot_normal())
        biases = _variable('biases', [hiddens], initializer=tf.keras.initializers.glorot_normal())
...
def res_block(x, a=None, filter_size=16, nonlinearity=concat_elu, keep_p=1.0, stride=1, gated=False, name="resnet"):
    ...
    if keep_p < 1.0:
        x_1 = tf.compat.v1.nn.dropout(x_1, keep_prob=keep_p)
    if not gated:
