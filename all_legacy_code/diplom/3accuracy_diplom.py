...
bound_files = sorted(os.listdir(bound_test_dir))
flow_files = sorted(os.listdir(flow_test_dir))

average_ratio = np.zeros(len(bound_files), dtype=np.float32)
max_ratio = []
time_gen = []
num_skipped = 0
sec_ratio = []
...
with tf.Graph().as_default():
    ...
    for num_file in range(len(bound_files)):

        print(num_file)
        bound_file = bound_test_dir + bound_files[num_file]
        flow_file = flow_test_dir + flow_files[num_file]
...
biggest_ARM = round(np.max(max_ratio) * 100, 3)
